...
class ListUtilities(commands.Cog):
    ...
    async def silence_warnings(self, ctx, message):
        ...
        if not is_offense_warning(message.content) and not is_utf_warning(message.content):
            await ctx.respond("This is not a warning message.", ephemeral = True)
            return

        word = get_original_word(message.content)
        if ignored_words.exists(word):
            await ctx.respond(f"**{word}** is already in the silenced warnings list.", ephemeral = True)
            return
    ...
    """
    import_grp = discord.SlashCommandGroup(name = "import", description = "Imports a list of words from a URL.")

    @import_grp.command(name = "profanities", description = "Imports a list of words to the profanity list.")
    async def import_profanities(self, ctx, url: str):
        ...
        async with httpx.AsyncClient() as client:
            ...
            await ctx.respond(f"Added {len(words)} words to the profanity list. Sample: {', '.join(words[:5])}")

    """
    This group shows stats for the selected list.
    """
    stats_grp = discord.SlashCommandGroup(name = "stats", description = "Shows stats for the selected list.")

    @stats_grp.command(name = "profanities", description = "Shows stats for the profanity list.")
    async def profanity_stats(self, ctx):
        count = profanity_list.count()
        await ctx.respond(f"Profanity list has {count} words.")

    @stats_grp.command(name = "ignored-words", description = "Shows stats for the ignored words list.")
    ...
    """
    add_grp = discord.SlashCommandGroup(name = "add", description = "Adds a word to the selected list.")
    ...
    async def add_profanity(self, ctx, word: str):
        ...
        if profanity_list.exists(word):
            await ctx.respond(f"**{word}** is already in the blacklist.", ephemeral = True)
            return

        profanity_list.add(word)
        await ctx.respond(f"Added **{word}** to the blacklist.")

    @add_grp.command(name = "ignored-words", description = "Adds a word to the ignored words list.")
    async def add_ignored_word(self, ctx, word: str):
        if not await user_can_run_command(ctx):
            return

        if ignored_words.exists(word):
            await ctx.respond(f"**{word}** is already in the ignored words list.", ephemeral = True)
            return
    ...
    """
    remove_grp = discord.SlashCommandGroup(name = "remove", description = "Removes a word from the selected list.")

    @remove_grp.command(name = "profanity", description = "Removes a word from the profanity list.")
    ...
    @remove_grp.command(name = "ignored-words", description = "Removes a word from the ignored words list.")
    async def remove_ignored_word(self, ctx, word: str):
        if not await user_can_run_command(ctx):
            return

        if not ignored_words.exists(word):
            await ctx.respond(f"**{word}** is not in the ignored words list.", ephemeral = True)
            return
